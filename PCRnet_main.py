...
def test_one_epoch(args, net, test_loader):
    ...
    with torch.no_grad():
        ...
        for data in tqdm(test_loader):
            ...
            src = src - torch.mean(src, dim=1, keepdim=True)
            ...
            translation_ab_pred = output['est_t'].transpose(2,1)
            ...
            translation_ba_pred = -torch.matmul(rotation_ba_pred, translation_ab_pred).squeeze(2).to(args.device)
            ...
            transformed_src = transform_point_cloud(src.transpose(2,1), rotation_ab_pred, translation_ab_pred.squeeze(2)).transpose(2,1)
            ...
            if args.loss == 'mse_transf':
                loss = torch.mean(torch.norm(torch.matmul(rotation_ab_pred.transpose(2, 1),rotation_ab)-identity, p='fro', dim=[1,2])**2) + torch.mean(torch.norm(translation_ab_pred - translation_ab, dim=1)**2) #+ torch.mean(torch.norm(sclr - sclrO, dim=1))
                ...
                loss = ChamferDistanceLoss()(target, output['transformed_source'])
...
# 	test_error_euler_angles_ab = error_euler_angles(test_rotations_ab_pred,np.degrees(test_eulers_ab))  # computing euler angle error
    ...
    np.savez_compressed('PCRnet_main_cranio', mse=np.asarray(mse), mae=np.asarray(mae), chamfer=np.asarray(chamfer), ErrorRot=test_error_euler_angles_ab, ErrorTran=error_trans)
...
def train(args, net, train_loader, test_loader, boardio, textio):
    ...
    # scheduler = lr_scheduler.StepLR(opt, step_size=30, gamma=0.7)
...
def options():
    ...
    parser.add_argument('--exp_name', type=str, default='PCRnet_exp_', metavar='N',
						help='Name of the experiment')
    parser.add_argument('--model', type=str, default='PCRnet', metavar='N',
						choices=['PCRnet'],
						help='Model to use, [PCRnet]')
    ...
    parser.add_argument('--loss', type=str, default='chamfer_dist', metavar='N',
						choices=['mse_transf', 'mse', 'chamfer_dist'],
...
def main():
    ...
    if args.model == 'PCRnet':
        ...
        net = iPCRNet(feature_model=ptnet)
