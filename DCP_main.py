    ...
    for data in tqdm(test_loader):
        ...
        if args.debug: # if degubbing
            for i in range(batch_size):
                ...
                np.savetxt("variables_storage/euler_ab_batch_{}_sample_{}".format(batch_idx,i),utl.npmat2euler(rotation_ab.cpu())[i,:])
        ...
        # if itr%10==0:
        #     utl.plotPCbatch(target.transpose(2,1).detach().cpu().numpy(), src.transpose(2,1).detach().cpu().numpy(), transformed_src.transpose(2,1).detach().cpu().numpy() , show=True, save=True, name = ("output/DCP_"+str(itr)), fig_count=1)
...
# 	test_error_euler_angles_ab = error_euler_angles(test_rotations_ab_pred,np.degrees(test_eulers_ab))  # computing euler angle error
    ...
    np.savez_compressed('DCP_main_cranio', mse=np.asarray(mse), mae=np.asarray(mae), chamfer=np.asarray(chamfer), ErrorRot=test_error_euler_angles_ab, ErrorTran=error_trans)
...
def train(args, net, train_loader, test_loader, boardio, textio):
    if args.use_sgd:
        print("Use SGD")
        opt = optim.SGD(net.parameters(), lr=args.lr, momentum=args.momentum, weight_decay=1e-4)
    else:
        print("Use Adam")
...
# 		train_error_euler_angles_ab = train_rotations_ab_pred-train_rotations_ab#error_euler_angles(train_rotations_ab_pred,np.degrees(train_eulers_ab))  # computing euler angle error
        ...
        test_error_euler_angles_ab = error_rotmat_angles(test_rotations_ab_pred,test_rotations_ab)
# 		test_error_euler_angles_ab = test_rotations_ab_pred-test_rotations_ab#error_euler_angles(test_rotations_ab_pred,np.degrees(test_eulers_ab))  # computing euler angle error
        test_r_mse_ab = np.mean(test_error_euler_angles_ab ** 2)
        test_r_rmse_ab = np.sqrt(test_r_mse_ab)
        test_r_mae_ab = np.mean(np.abs(test_error_euler_angles_ab))
        test_t_mse_ab = np.mean((test_translations_ab - test_translations_ab_pred) ** 2)
...
def create_pcd_obj(np_array,col=[1,0,0]):
	...
	return pcd
# DCP_exp_b8_e200_modelN_WO-lrS_20240223-175708
def options():
    timestamp = datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
    parser = argparse.ArgumentParser(description='Point Cloud Registration')
    parser.add_argument('--exp_name', type=str, default='DCP_exp_', metavar='N',
...
def main():
    ...
    if args.dataset == 'modelnet40':
        ...
        train_dataset = RegistrationData(ModelNet40Data(train=True))
        test_dataset = RegistrationData(ModelNet40Data(train=False))
